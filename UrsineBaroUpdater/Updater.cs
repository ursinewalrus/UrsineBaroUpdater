...
namespace UrsineBaroUpdater
{
    class Updater
    {
        ...
        {
            if (File.Exists(@".\fakeBaro.zip"))
            {
                File.Delete(@".\fakeBaro.zip");
            }
            CreateZip(@".\Barotrauma", @".\fakeBaro.zip");
        ...
        public static void CreateZip(string inputDir, string outputDir)
        {
            ZipFile.CreateFromDirectory(inputDir, outputDir);
        }
